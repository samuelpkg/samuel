...
```go
// samuel_v2/internal/orchestrator/setup.go
...
`samuel_v2/docs/getting-started/migration-v1.md` includes a section explaining the gstack/gbrain change:
...
- [ ] `samuel_v2/internal/components/` contains only `samuel/` — no `gstack/` or `gbrain/` packages.
- [ ] No source file in `samuel_v2/internal/` mentions `gstack` or `gbrain` as a string literal (CI grep check, similar to the agnostic-by-design check).
