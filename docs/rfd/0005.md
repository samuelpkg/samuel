...
3. Define `Plugin` interface at `internal/plugin/plugin.go`:
...
6. Port `Orchestrator` from `samuel_v1/internal/orchestrator/orchestrator.go` to `internal/orchestrator/`. Keep:
...
8. Port `SamuelComponent` from `samuel_v1/internal/orchestrator/component_samuel.go` to `internal/components/samuel/`. Adapt for v2:
...
10. Implement `SkillPlugin` at `internal/plugin/skill/`. `Install` clones the plugin's Git repo at the resolved tag, verifies cosign signature on the archive, copies SKILL.md + scripts + references + assets to `<project>/.samuel/plugins/<name>/`.

11. Implement `WasmPlugin` at `internal/plugin/wasm/`. Uses wazero. `Install` fetches the WASM blob, verifies signature, stores in `<project>/.samuel/plugins/<name>/plugin.wasm`. `Check` instantiates the module and calls its `health()` export.

12. Implement `OciPlugin` at `internal/plugin/oci/`. Detects container runtime (Podman rootless → Docker). `Install` pulls the image, pins digest in `samuel.lock`. `Check` runs `<runtime> inspect <image>`.
...
14. Build a `FakePlugin` test type at `internal/plugin/testutil/` that satisfies `Plugin` with configurable failure injection. Used to test:
...
4. **Plugin protocol invocation**: split by tier.
   ...
   - **OCI plugins**: gRPC over Unix socket via the `/samuel-bridge` mount. Protobuf schema at `api/proto/plugin/v1/`. Per [[0001|RFD 0001]] resolved decision #3.
